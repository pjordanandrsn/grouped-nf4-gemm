...
| **f32 compute** (`compute="f32"`, split or packed kernel) | `fp8-paged-attention-f32-compute` | sm_80–sm_88 by default; sm_89+ whenever a constraint above fails with the env unset; every explicit `GNF4_ATTN_COMPUTE=f32` / `compute="f32"` on any card | sm_80 or newer | RTX A2000 (sm_86) and RTX 5090 (sm_120), 2026-09-21, arms pinned to the mode they name (claim `gnf4.serve.f32-arms-ran-fp8`, measured) | supported — [#319](https://github.com/pjordanandrsn/grouped-nf4-gemm/issues/319) is closed: its failures were the shape arms running the fp8 kernel while judged at the f32 tolerance, never a defect in these kernels (the earlier claim `gnf4.open.f32-compute-modes-triton34` is **retired**) |
...
- The f32 compute modes are measured on sm_86 and sm_120 and meet their tolerance on both (`gnf4.serve.f32-arms-ran-fp8`). `#319` reported them failing on sm_120; the claim `gnf4.open.f32-compute-modes-triton34` recording that is **retired**, because the suite's f32 shape arms passed no `compute` kwarg and so ran the **fp8** kernel there while being judged at the f32 tolerance. The advice that issue produced — gate a lane with `-k "f8dot or pf8"` — excluded exactly the mislabelled arms and should not be followed.
