...
(**anchored** documents are never edited in place). Corrections to anchored
documents live in [`kernel/ERRATA.md`](../kernel/ERRATA.md); the timing erratum
of the 2026-08-15 protocols is in
...
| doc | note |
|---|---|
| [`cold-engine/PHASE0-premise.md`](cold-engine/PHASE0-premise.md) | the premise test — **refuted** on the target box; its header still says the pipe term is pending, but its §0.4 table carries it measured (5.83 GB/s pinned); the fat-host DDR column stays an estimate |
...
The serving-side kernels shipped from 0.14 onward — fp8 paged decode
attention and its compute modes, the int4-b32 GEMV/GEMM and calibrated
packer, the small-M int4 GEMM (`int4_smallm`), the decode glue kernels,
the sm_120 census, the decode anchor — have their reference pages under
[`solutions/`](solutions/)
([`fp8-paged-attention-for-moe-serving.md`](solutions/fp8-paged-attention-for-moe-serving.md),
[`int4-decode-gemv.md`](solutions/int4-decode-gemv.md)); their history is
in `CHANGELOG.md` and their receipts in `kernel/RESULTS-*.md` and
`bench/sm120-census/`. `STATUS.md` and `claims.json` cover them.
...
anchored) with the MI300X results beside them
(`RESULTS-multiarch-mi300x.md`, `RESULTS-multiarch-mi300x-followup.md`,
both anchored).

`router_probe/` is the router-predictability probe, at exploratory tier:
`CHARTER.md`, `RESULTS.md` and `procedure.yaml` are anchored, as are five of
its receipts, and no number in it is registered in `claims.json`. Its
RESULTS.md still says the remaining families are pending, although it now
carries Qwen3-30B-A3B, gpt-oss-20b, gpt-oss-120b and Granite sections; it has
no Qwen3-235B-A22B section or receipt.

`sycl/` (`README.md`, `M2-RESULTS.md`) is the SYCL decode-GEMV port's working
record (2026-07-15); none of its numbers is registered in `claims.json`.

Also outside `docs/`: `kernel/GATE2-ASSESSMENT.md` (the 2026-07-13
flip-public decision memo; its addendum defers to
`kernel/RESULTS-gate2-confirmatory.md`), `kernel/ATTESTATION-TIMELINE-2026-08-15.md`
(what the 2026-08-15 stamps prove, with one erratum), `kernel/ERRATA.md`
(corrections to anchored documents) and `ATTRIBUTION.md`.
