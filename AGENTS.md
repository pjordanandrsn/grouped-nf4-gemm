...
python scripts/check_claims_register.py            # docs/claims.json against docs/claims-schema.md (one schema, the same file in both repositories); --sibling ../experts4bit-qlora
...
- A task pull request never moves a gate, a threshold, a compiled-kernel default
  ...
  **Kernel changes need the GPU tests run on the named hardware, with the receipt
  in the pull request.**
- Sections 2–9 bind every contributor equally.
