...
static int gemv_common(GemvCtx *c, int G, int threads, pool_fn range,
    ...
    int rs = 1;
    /* Re-certification revision (PREREG-lowg-split2): target ~2x threads,
     * not >= threads -- the first cert measured the N=768/G=1 cell stuck
     * at rs=2 (48 items over 32 threads: 1.5 ragged waves, no win) while
     * the N=256 cell at 4x items delivered 1.8x. Balance needs headroom. */
    while (items * rs < 2 * thr_hint && rs < 64) rs <<= 1;
    c->rsplit = rs;
