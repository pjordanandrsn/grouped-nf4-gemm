...
from mxfp4_grouped import FP4_VALUES  # noqa: E402  (imports on every platform via the shim)


def test_e2m1_decode_table():
    """The kernel's ``2*value`` integer decode, evaluated in python for
    every nibble, is the codebook. Runs everywhere -- no triton needed."""
    for nib in range(16):
        e, m = (nib >> 1) & 3, nib & 1
