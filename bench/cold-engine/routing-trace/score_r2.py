...
def main():
    ...
    ap.add_argument("--protected", default="quarter,half,three-quarter",
                    help="budgets as a fraction of rows. R3 showed the "
                         "resurrection rate is decided by this, so a single "
                         "budget would say nothing.")
    ...
    print(f"\n{'rows':>5} {'prot':>6} {'resurr':>8} {'overwr':>8} "
          f"{'/routed (R2)':>13} {'/resolved (R3)':>15}")
    out = {"meta": meta, "points": []}
    for rows in [int(x) for x in a.rows.split(",")]:
        for name in a.protected.split(","):
            prot = max(1, int(frac[name] * rows))
            try:
                p = replay(recs, rows, prot)
            except ValueError as exc:                  # refused budget
                print(f"{rows:>5} {prot:>6}  refused: {str(exc)[:44]}")
                continue
            p["budget"] = name
            out["points"].append(p)
            pr = p["per_resolved_eviction"]
            print(f"{rows:>5} {prot:>6} {p['resurrections']:>8} "
