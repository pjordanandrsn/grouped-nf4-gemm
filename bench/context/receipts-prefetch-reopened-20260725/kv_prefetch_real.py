...
# SEEDED. Unseeded, every arm in a run shares a prompt but runs do not, so the
# token-divergence comparison is meaningless ACROSS runs -- which #18 recorded
# after reading "diverges at 1" and "never diverges" as a result. P1c cannot be
# scored without this.
SEED = int(os.environ.get("E1_SEED", "0"))
...
def main():
    ...
    ids = torch.randint(100, 20000, (1, PROMPT_LEN), device="cuda", generator=g)
    ...
    # P1 of PREREG-prefetch-reopened.md, scored on the same three arms
    p1a = pre / plain
    p1b = hidden
    p1c = toks["streamed"] == toks["streamed+pre"]
    print("\n=== P1 (prefetch reopened) ===")
    print(f"P1a prefetched/streamed = {p1a:.3f}   <=0.95 confirms, >=1.00 falsifies  "
          f"{'CONFIRMED' if p1a <= 0.95 else ('FALSIFIED' if p1a >= 1.00 else 'outside interval')}")
    print(f"P1b hidden fraction     = {p1b * 100:.1f}%   >=25% confirms, <0% falsifies  "
          f"{'CONFIRMED' if p1b >= 0.25 else ('FALSIFIED' if p1b < 0 else 'outside interval')}")
    print(f"P1c ids identical       = {p1c}   {'CONFIRMED' if p1c else 'FALSIFIED'}")
    v = {
        "P1a": dict(measured=p1a), "P1b": dict(measured=p1b), "P1c": dict(ok=p1c),
