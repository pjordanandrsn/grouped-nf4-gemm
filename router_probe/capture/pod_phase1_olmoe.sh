#!/bin/bash
# Phase-1 cheap-census, OLMoE only, on ONE cloud GPU (>=16GB). Uses the Experts4bit
# streaming loader (bundled experts4bit_qlora) so the FUSED OlmoeExperts stacks are
# NF4-quantized (stock load_in_4bit's nn.Linear walker skips them, bitsandbytes#1849).
S=/root/RP_STATE
